...
def pixel_to_gps(drone_lat, drone_lon, alt, yaw_deg, target_cx, target_cy):
    ...
    """
    print(f"[GEO][pixel_to_gps] Giriş: drone=({drone_lat:.6f},{drone_lon:.6f}) "
    ...
    print(f"[GEO][pixel_to_gps] Merkeze göre piksel ofseti: dx={delta_x_pixel:.1f} dy={delta_y_pixel:.1f}")

    offset_angle_x = delta_x_pixel * (fov_x_rad / config.WIDTH)
    offset_angle_y = delta_y_pixel * (fov_y_rad / config.HEIGHT)
    print(f"[GEO][pixel_to_gps] Açısal ofset: ax={math.degrees(offset_angle_x):.3f}° "
    ...
    print(f"[GEO][pixel_to_gps] Zemine projeksiyon: ileri={dist_y:.2f}m sağ={dist_x:.2f}m")
    ...
    print(f"[GEO][pixel_to_gps] Dünya koordinatları: kuzey={delta_north:.2f}m doğu={delta_east:.2f}m")

    target_lat = drone_lat + (delta_north / 111320)
    target_lon = drone_lon + (delta_east  / (111320 * math.cos(math.radians(drone_lat))))
    print(f"[GEO][pixel_to_gps] Sonuç: hedef GPS=({target_lat:.6f}, {target_lon:.6f})")
...
def calculate_drop_point(target_lat, target_lon, alt, speed_ms, yaw_deg):
    ...
    """
    print(f"[GEO][calculate_drop_point] Hedef: ({target_lat:.6f},{target_lon:.6f}) "
    ...
    print(f"[GEO][calculate_drop_point] Düşüş süresi={fall_time:.3f}s | "
    ...
    print(f"[GEO][calculate_drop_point] Geri ofset: kuzey={delta_north:.2f}m doğu={delta_east:.2f}m")

    release_lat = target_lat + (delta_north / 111320)
    release_lon = target_lon + (delta_east  / (111320 * math.cos(math.radians(target_lat))))
    print(f"[GEO][calculate_drop_point] Release point: ({release_lat:.6f}, {release_lon:.6f})")
